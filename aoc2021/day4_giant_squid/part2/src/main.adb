...
procedure Main is
   ...
   type Cards is array (1 .. 100) of Table;
   ...
   begin
      for N in 1 .. Num_Size loop
         ...
         for Card of Bingo_Cards loop
            for I in Card.Board'Range(1) loop
               for J in Card.Board'Range(2) loop
                  if Card.Board(I, J).Value = Numbers (N) and Card.Won /= True then
                     Card.Board(I, J).Mark := True;

                     if (Check_Card (Card.Board) = True) then

                        Cards_Won := Cards_Won + 1;
                        Card.Won := True;

                        New_Line;
                        Put_Line ("Winner " & Cards_Won'Image);
                        Print_Card (Card.Board);

                        if Cards_Num = Cards_Won then
                           New_Line;
                           Put_Line ("Last Winner:");
                           Print_Card (Card.Board);
                           return Calc_Score (Card.Board, Numbers (N));
                        end if;
                     end if;
                  end if;
               end loop;
...
begin
   ...
   while not End_Of_File (Input_File) loop
      for Card of Players_Cards loop
         for I in Card.Board'Range(1) loop
            for J in Card.Board'Range(2) loop
               Get (Input_File, Card.Board(I, J).Value);
            end loop;
         end loop;
